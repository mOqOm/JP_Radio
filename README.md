...
+ 2026/08/17 タイムフリー番組表の出演者・放送局の表示順を元に戻す(8/16の変更がかえって見にくかったため。局名を常時表示に)
+ 2026/08/17 お気に入り番組(タイムフリー)の選択時、専用の管理モーダルを開く実装をやめ、通常のタイムフリー番組表と全く同じ表示・再生導線(browseMode2設定に従う)に統一
+ 2026/08/17 出演者情報が無い番組の表示が詰まって見えないよう、半角スペースで埋めるように変更
